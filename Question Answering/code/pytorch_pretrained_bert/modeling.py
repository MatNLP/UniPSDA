...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True,  encode_replacedEmbedding=None):
        ...
        if encode_replacedEmbedding is None:
            encoded_layers = self.encoder(embedding_output,
                                          extended_attention_mask,
                                          output_all_encoded_layers=output_all_encoded_layers)
        else:
            encoded_layers = self.encoder(encode_replacedEmbedding,
                                          extended_attention_mask,
                                          output_all_encoded_layers=output_all_encoded_layers)
...
path = '../nq-train_questionWithSEP.json'
with open(path, "r") as f:
    train_questionWithSEP_list = json.load(f)
    print()
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(self, batch_with_SEP, batch_sen_pos, batch_candidate_embeddings_list, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):

        if batch_candidate_embeddings_list is not None:
            input_ids_with_SEP, input_mask_with_SEP, segment_ids_with_SEP, start_positions_with_SEP, end_positions_with_SEP = batch_with_SEP
            start_positions_with_SEP = start_positions_with_SEP.cuda()
            end_positions_with_SEP = end_positions_with_SEP.cuda()
            ...
            utt_replace_str = sequence_output[2]
            tmp_collect_with_SEP = self.bert(input_ids_with_SEP.cuda(), segment_ids_with_SEP.cuda(),
                                             input_mask_with_SEP.cuda(), output_all_encoded_layers=False,
                                             encode_replacedEmbedding=None)
            utt = tmp_collect_with_SEP[2]
            # batch_sens_sim = []
            # total_window_sim_list = []
            total_distance_from_SVD_list = []
            total_sigma_last_k_list = []
            total_Wasserstein_Distance = []
            # batch_W_list = []
            #########################
            for i in range(len(batch_sen_pos)):
                # each_sen_sim = []
            ...
            utt = self.bert(input_ids_with_SEP.cuda(), segment_ids_with_SEP.cuda(), input_mask_with_SEP.cuda(), \
                            output_all_encoded_layers=False, encode_replacedEmbedding=utt)
        else:
            tmp_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False,
                                        encode_replacedEmbedding=None)
            utt = tmp_output[2]
            utt = self.bert(input_ids, token_type_ids, attention_mask, \
                            output_all_encoded_layers=False, encode_replacedEmbedding=utt)



        # logits = self.qa_outputs(utt[1])
        logits_2 = self.qa_outputs(utt[0])
        ...
        if start_positions is not None and end_positions is not None:
            ...
            loss_fct = CrossEntropyLoss()
            start_loss_2 = loss_fct(start_logits_2, start_positions_with_SEP)
            # start_loss = loss_fct(torch.sigmoid(start_logits), start_positions.squeeze(-1))
            # end_loss = loss_fct(torch.sigmoid(end_logits), end_positions.squeeze(-1))
            end_loss_2 = loss_fct(end_logits_2, end_positions_with_SEP)
            # end_loss = loss_fct(end_logits, end_positions)

            # total_loss_0 = (start_loss + end_loss) / 2
            total_loss_0 = (start_loss_2 + end_loss_2) / 2 / start_logits_2.size(0)
            total_loss = 0.6 * total_loss_0 + 0.4*(0.4* mean_distance_from_SVD + 0.2*self.eta*mean_last_k_sigma_sum + 0.4*mean_Wasserstein_Distance)
            return total_loss
        else:
            # return start_logits, end_logits
            return start_logits_2, end_logits_2
