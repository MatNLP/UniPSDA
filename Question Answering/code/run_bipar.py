...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        if len(all_doc_tokens) >= (max_seq_length - 3 - (len(query_tokens) + 3)):
            all_doc_tokens = all_doc_tokens [: (max_seq_length - 3 - (len(query_tokens) + 3)) ]
            num_len_doc_span_is_2 = num_len_doc_span_is_2 + 1
...
def make_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    all_examples = all_examples[:len(all_results)]
    for (example_index, example) in enumerate(all_examples):
        if example_index == 1494:
            print()
...
def evaluate(args, model, device, eval_dataset, eval_dataloader,
    ...
    for idx, (input_ids, input_mask, segment_ids, example_indices) in enumerate(eval_dataloader):
        ...
        with torch.no_grad():
            batch_start_logits, batch_end_logits = model(None, None, None,input_ids, segment_ids, input_mask)
...
def load_batch_candidate_embeddings(iter, batchsize, senNumKey_wordsHiddenListValue, senNumKey_candidateTensorValue):
...
def main(args):
    ...
    if args.do_train or (not args.eval_test):
        ...
        eval_dataloader = DataLoader(eval_data, batch_size=args.eval_batch_size,drop_last=True)

    if args.do_train:

        # =========================with SEP input_ids=================================
        train_examples_with_SEP = read_squad_examples(
            input_file='../data/BiPaR/Monolingual_EN_trainWithSEP.json',
            is_training=True, version_2_with_negative=args.version_2_with_negative)

        train_features_with_SEP = convert_examples_to_features(
            examples=train_examples_with_SEP,
            tokenizer=tokenizer,
            max_seq_length=args.max_seq_length,
            doc_stride=args.doc_stride,
            max_query_length=args.max_query_length,
        ...
        if args.train_mode == 'sorted' or args.train_mode == 'random_sorted':
            train_features_with_SEP = sorted(train_features_with_SEP, key=lambda f: np.sum(f.input_mask))
        else:
            random.shuffle(train_features_with_SEP)
        all_input_ids_with_SEP = torch.tensor([f.input_ids for f in train_features_with_SEP], dtype=torch.long)
        all_input_mask_with_SEP = torch.tensor([f.input_mask for f in train_features_with_SEP], dtype=torch.long)
        all_segment_ids_with_SEP = torch.tensor([f.segment_ids for f in train_features_with_SEP], dtype=torch.long)
        all_start_positions_with_SEP = torch.tensor([f.start_position for f in train_features_with_SEP],
                                                    dtype=torch.long)
        all_end_positions_with_SEP = torch.tensor([f.end_position for f in train_features_with_SEP], dtype=torch.long)
        train_data_with_SEP = TensorDataset(all_input_ids_with_SEP, all_input_mask_with_SEP, all_segment_ids_with_SEP,
                                            all_start_positions_with_SEP, all_end_positions_with_SEP)
        train_dataloader_with_SEP = DataLoader(train_data_with_SEP, batch_size=args.train_batch_size,drop_last=True)
        train_batches_with_SEP = [batch_with_SEP for batch_with_SEP in train_dataloader_with_SEP]
        # =========================with SEP input_ids==================================
        ...
        for lr in lrs:
            ...
            write_path = '../data/BiPaR/processed_data_bipar/senNumKey_wordsHiddenListValue_qa.pt'
            senNumKey_wordsHiddenListValue = torch.load(write_path)
            path = '../data/BiPaR/processed_data_bipar/senNumKey_candidateTensorValue_qa.pt'
            ...
            for epoch in range(int(args.num_train_epochs)):
                ...
                for step, batch in enumerate(train_batches):
                    ...
                    batch_with_SEP = train_batches_with_SEP[step]
                    input_ids_with_SEP, input_mask_with_SEP, segment_ids_with_SEP, \
                            start_positions_with_SEP, end_positions_with_SEP = batch_with_SEP
                    batch_candidate_embeddings_list = load_batch_candidate_embeddings(step, args.train_batch_size,
                                                                                           senNumKey_wordsHiddenListValue,
                                                                                           senNumKey_candidateTensorValue)
                    batch_sen_pos = pos_102_list(input_ids_with_SEP)
                    loss = model(batch_with_SEP, batch_sen_pos,batch_candidate_embeddings_list, input_ids, segment_ids, input_mask, start_positions, end_positions, )
                    if n_gpu > 1:
                        loss = loss.mean()
                    if args.gradient_accumulation_steps > 1:
    ...
    if args.do_eval:
        ...
        logger.info("!!! Test exact_match : %.2f" % ( result_full['exact']))
        logger.info("!!! Test f1: %.2f" % ( result_full[args.eval_metric]))
        with open(os.path.join(args.output_dir, "predictions.json"), "w") as writer:
            writer.write(json.dumps(preds_full, indent=4) + "\n")


if __name__ == "__main__":
        parser = argparse.ArgumentParser()
        parser.add_argument("--model", default='bert-base-multilingual-cased', type=str)
        parser.add_argument("--output_dir", default='squad_output', type=str,
                            help="The output directory where the model checkpoints and predictions will be written.")
        parser.add_argument("--train_file", default='../data/BiPaR/Monolingual_EN_train.json', type=str,
                            help="SQuAD json for training. E.g., train-v1.1.json")
        parser.add_argument("--dev_file", default='../data/BiPaR/Monolingual_ZH_valid.json', type=str,
                            help="SQuAD json for predictions. E.g., dev-v1.1.json or test-v1.1.json")
        parser.add_argument("--test_file", default='../data/BiPaR/Monolingual_ZH_test.json', type=str)
        ...
        parser.add_argument("--eval_test", default=True, action='store_true', help='Wehther to run eval on the test set.')
        ...
        parser.add_argument("--train_mode", type=str, default='sorted', choices=['random', 'sorted', 'random_sorted'])
