...
class BERTEncoder(nn.Module):
    ...
    def forward(self,  token, att_mask, pos1, pos2, encode_replacedEmbedding=None):
        """
        Args:
        ...
        if encode_replacedEmbedding is not None:
            tmp = self.bert(token, attention_mask=att_mask, return_dict=False, encode_replacedEmbedding=encode_replacedEmbedding)
            hidden = tmp[1]
        else:
            tmp = self.bert(token, attention_mask=att_mask, return_dict=False,
                            encode_replacedEmbedding=encode_replacedEmbedding)
            hidden = tmp[2]
...
class BERTEntityEncoder(nn.Module):
    ...
    def forward(self, token, att_mask, pos1, pos2):
        ...
        tmp = self.bert(token, attention_mask=att_mask, return_dict=False, encode_replacedEmbedding=None)
        hidden = tmp[2]
