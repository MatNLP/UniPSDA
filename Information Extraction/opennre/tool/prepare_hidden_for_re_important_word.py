...
def important_words_hidden( bert, tokenizer):
    ...
    write_path = '../senNumKey_posAndWordAndRootValue_qa.pt'
    senNumKey_posAndWordValue = torch.load(write_path)
    ...
    write_path = '../senNumKey_wordsHiddenListValue_qa.pt'
    torch.save(senNumKey_wordsHiddenListValue, write_path)
    print('write done')
    return senNumKey_wordsHiddenListValue


def important_words_hidden_re( bert, tokenizer):
    write_path = '../senNumKey_posAndWordAndRootValue_re.pt'
    senNumKey_posAndWordValue = torch.load(write_path)
    ...
    write_path = '../senNumKey_wordsHiddenListValue_re.pt'
    torch.save(senNumKey_wordsHiddenListValue, write_path)
    print('write done')
    return senNumKey_wordsHiddenListValue
...
def important_word_in_en_vocab_index_re():
    ...
    for senNum in senNumKey_posAndWordValue.keys():
        ...
        for word_str in senNumKey_posAndWordValue[senNum]['word']:
            ...
            sen1_vocab_index.append(vocab_index)
        senNumKey_posAndWordValue[senNum]['vocab_index'] = sen1_vocab_index
    path = '../senNumKey_posAndWordAndRootAndVocabIndexValue_re.pt'
    torch.save(senNumKey_posAndWordValue, path)


def important_word_in_cluster30000_typeIndex_re():
    path = '../senNumKey_posAndWordAndRootAndVocabIndexValue_re.pt'
    senNumKey_posAndWordAndRootAndVocabIndexValue = torch.load(path)
    lang = 'en'
    path1 = '../type_30000_tensor_%s.pt' % lang
    type_30000_tensor_lang = torch.load(path1)
    path2 = '../type_30000_index_of_big_dict_%s.pt' % lang
    ...
    for senNum in senNumKey_posAndWordAndRootAndVocabIndexValue.keys():
        if senNum % 1000 == 0:
            time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            print("senNum: %d || %s " % (senNum, time))
        sen1_cluster_30000_type_index_list = []
        for index in senNumKey_posAndWordAndRootAndVocabIndexValue[senNum]['vocab_index']:
            if index in en_index2type_30000.keys():
                ...
                sen1_cluster_30000_type_index_list.append(cluster_30000_type_index)
            else:
                cluster_30000_type_index = 30000
                sen1_cluster_30000_type_index_list.append(cluster_30000_type_index)
        senNumKey_posAndWordAndRootAndVocabIndexValue[senNum]['30000_type_index'] = sen1_cluster_30000_type_index_list
    path = '../senNumKey_posAndWordAndRootAndVocabIndexAnd30000typeIndexValue_re.pt'
    torch.save(senNumKey_posAndWordAndRootAndVocabIndexValue, path)
    print()


def find_same_cluster_in_de_en_re():
    lang = 'en'
    path2 = '../type_30000_index_of_big_dict_%s.pt' % lang
    ...
    for senNum in senNumKey_posAndWordAndRootAndVocabIndexAnd30000typeIndexValue.keys():
        senNumKey_30000typeIndexValue[senNum] = {}
        if senNum % 1000 == 0:
            time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            print("senNum: %d || %s " % (senNum, time))
        sen1_cluster_30000_type_index_de_en_list = []
        for index in senNumKey_posAndWordAndRootAndVocabIndexAnd30000typeIndexValue[senNum]['30000_type_index']:
            if index in de_en_index2type_30000.keys():
                ...
                sen1_cluster_30000_type_index_de_en_list.append(type_index)
        senNumKey_30000typeIndexValue[senNum] = sen1_cluster_30000_type_index_de_en_list
    path = '../senNumKey_cluster_30000_type_index_de_en_re.pt'
    torch.save(senNumKey_30000typeIndexValue, path)
    print()

def find_same_cluster_in_multi_lang_re():
    path2 = '../type_30000_index_of_big_dict_multi_lang_real.pt'
    ...
    for senNum in senNumKey_cluster_30000_type_index_de_en.keys():
        senNumKey_30000typeIndexValue_multi_lang[senNum]= {}
        if senNum % 1000 == 0:
            time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            print("senNum: %d || %s " % (senNum, time))
        sen1_cluster_30000_type_index_multi_lang_list = []
        for index in senNumKey_cluster_30000_type_index_de_en[senNum]:
            if index in multi_lang_index2type_30000.keys():
                ...
                sen1_cluster_30000_type_index_multi_lang_list.append(type_index)
        senNumKey_30000typeIndexValue_multi_lang[senNum] = sen1_cluster_30000_type_index_multi_lang_list
    path = '../senNumKey_30000typeIndexValue_multi_lang_re.pt'
    torch.save(senNumKey_30000typeIndexValue_multi_lang, path)
    print()



def find_important_word_candidate_tensor_re():
    path1 = '../tensor_30000_collect_from_multi_lang_real.pt'
    ...
    for senNum in senNumKey_30000typeIndexValue_multi_lang.keys():
        senNumKey_candidateTensorValue[senNum] = {}
        sen1_candidateTensor = []
        for index in senNumKey_30000typeIndexValue_multi_lang[senNum]:
            ...
            sen1_candidateTensor.append(candidate_tensor)
        senNumKey_candidateTensorValue[senNum] = sen1_candidateTensor
...
if __name__ == '__main__':
    ...
    # important_words_hidden_re(bert, tokenizer)

    # important_word_in_en_vocab_index_re()

    # important_word_in_cluster30000_typeIndex_re()

    # find_same_cluster_in_de_en_re()

    # find_same_cluster_in_multi_lang_re()

    find_important_word_candidate_tensor_re()