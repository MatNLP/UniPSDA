...
class List(object):
    ...
    def to_bert_info(inputs, tokener, pad, cls, device, max_len = 256):
        ...
        batch_sen_pos = util.convert.List.pos_102_list(torch.Tensor(source).long())
        return (torch.Tensor(source).long().to(device), torch.Tensor(source_idx).long().to(device), torch.Tensor(source_msk).long().to(device)), source_len, batch_sen_pos
